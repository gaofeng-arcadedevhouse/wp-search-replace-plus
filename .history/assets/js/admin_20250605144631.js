jQuery(document).ready(function($) {
    ...
    var WTSR = {
        ...
        bindEvents: function() {
            ...
            // Debug functionality
            $('#wtsr-toggle-debug').on('click', this.toggleDebugInfo.bind(this));
            $('#wtsr-test-ajax').on('click', this.testAjaxConnection.bind(this));
        ...
        initializePaginationControls: function() {
            var paginationHtml = '<div class="wtsr-pagination-controls" style="display: none;">' +
                '<div class="wtsr-pagination-info">' +
                    '<label for="wtsr-results-per-page">每页显示: </label>' +
                ...
                '<div class="wtsr-pagination-buttons">' +
                    '<button type="button" class="button wtsr-pagination-btn" data-action="first" disabled>首页</button>' +
                    '<button type="button" class="button wtsr-pagination-btn" data-action="prev" disabled>上一页</button>' +
                    '<span class="wtsr-page-info">第 <span class="current-page">1</span> 页，共 <span class="total-pages">1</span> 页</span>' +
                    '<button type="button" class="button wtsr-pagination-btn" data-action="next" disabled>下一页</button>' +
                    '<button type="button" class="button wtsr-pagination-btn" data-action="last" disabled>末页</button>' +
                '</div>' +
            '</div>';
        ...
        updatePaginationControls: function() {
            ...
            $('.wtsr-results-count').text('显示 ' + startResult + '-' + endResult + ' 条，共 ' + this.totalResults + ' 条结果');
        ...
        renderResults: function(results, startIndex) {
            ...
            if (results.length === 0) {
                $('#wtsr-results-table').html('<p>当前页面没有结果</p>');
                return;
            }
        ...
        updateDebugInfo: function() {
            $('#debug-ajax-url').text(wtsr_ajax.ajax_url);
            $('#debug-nonce').text(wtsr_ajax.nonce);
        },
        
        toggleDebugInfo: function(e) {
            e.preventDefault();
            var $debugInfo = $('#wtsr-debug-info');
            var $toggleBtn = $('#wtsr-toggle-debug');
            
            if ($debugInfo.is(':visible')) {
                $debugInfo.hide();
                $toggleBtn.text('显示调试信息');
            } else {
                $debugInfo.show();
                $toggleBtn.text('隐藏调试信息');
                this.updateDebugInfo();
            }
        },
        
        testAjaxConnection: function(e) {
            e.preventDefault();
            var self = this;
            
            console.log('WTSR: Testing AJAX connection...');
            self.showMessage('正在测试AJAX连接...', 'info');
            
            // Test our custom endpoint first
            $.post(wtsr_ajax.ajax_url, {
                action: 'wtsr_test',
                nonce: wtsr_ajax.nonce
            })
            .done(function(response) {
                console.log('WTSR: Test endpoint response:', response);
                if (response.success) {
                    self.showMessage('AJAX连接测试成功: ' + response.data.message, 'success');
                } else {
                    self.showMessage('AJAX测试失败: ' + (response.data ? response.data.message : 'Unknown error'), 'error');
                }
            })
            .fail(function(xhr, status, error) {
                console.error('WTSR: Test endpoint failed:', xhr, status, error);
                ...
                self.showMessage('AJAX连接测试失败: ' + errorDetails, 'error');
            });
        },
        ...
        handleSearch: function(e) {
            ...
            // Check if ACF is selected but not available
            if (searchScope.includes('acf') && $('#search_acf').is(':disabled')) {
                this.showMessage('ACF字段搜索需要安装并激活Advanced Custom Fields插件', 'warning');
                $('#search_acf').prop('checked', false);
                return;
            }
        ...
        displaySearchResults: function(data) {
            ...
            // Update summary
            $('#wtsr-summary-text').text(data.summary || '找到 ' + this.totalResults + ' 个匹配项');
        ...
        performReplace: function() {
            ...
            if (selectedItemsData.length === 0) {
                ...
                this.showMessage('没有有效的项目可以替换', 'error');
                $button.prop('disabled', false).text(originalText);
                this.hideProgress();
                return;
            }
