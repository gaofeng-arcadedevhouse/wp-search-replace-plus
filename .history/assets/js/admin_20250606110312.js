jQuery(document).ready(function($) {
    ...
    var WTSR = {
        ...
        showReplaceHistory: function(e) {
            ...
            // Show loading
            var historyHtml = '<div id="wtsr-history-modal" class="wtsr-modal" style="display: block;">';
            historyHtml += '<div class="wtsr-modal-content" style="max-width: 800px;">';
            historyHtml += '<div class="wtsr-modal-header">';
            historyHtml += '<h3>Replace History</h3>';
            historyHtml += '<button type="button" class="wtsr-modal-close">&times;</button>';
            historyHtml += '</div>';
            historyHtml += '<div class="wtsr-modal-body">';
            historyHtml += '<p>Loading history...</p>';
            historyHtml += '</div>';
            historyHtml += '</div>';
            historyHtml += '</div>';
            ...
            $.post(wtsr_ajax.ajax_url, {
                action: 'wtsr_get_history',
                nonce: wtsr_ajax.nonce
            })
            .done(function(response) {
                if (response && response.success && response.data) {
                    self.renderReplaceHistory(response.data);
                } else {
                    $('.wtsr-modal-body').html('<p>No replace history found.</p>');
                }
            })
            .fail(function() {
                $('.wtsr-modal-body').html('<p>Failed to load replace history.</p>');
            });
        },
        
        renderReplaceHistory: function(history) {
            var historyHtml = '';
            
            if (!history || history.length === 0) {
                historyHtml = '<p>No replace operations found.</p>';
            } else {
                historyHtml = '<div class="wtsr-history-list">';
                history.forEach(function(item, index) {
                    historyHtml += '<div class="wtsr-history-item" style="border: 1px solid #ddd; margin-bottom: 10px; padding: 15px;">';
                    historyHtml += '<div style="display: flex; justify-content: space-between; align-items: flex-start;">';
                    historyHtml += '<div>';
                    historyHtml += '<h4 style="margin: 0 0 10px 0;">Operation #' + (index + 1) + '</h4>';
                    historyHtml += '<p><strong>Date:</strong> ' + (item.completed_at || item.created_at || 'Unknown') + '</p>';
                    historyHtml += '<p><strong>Search:</strong> "' + (item.search_text || '') + '"</p>';
                    historyHtml += '<p><strong>Replace:</strong> "' + (item.replace_text || '') + '"</p>';
                    historyHtml += '<p><strong>Items Affected:</strong> ' + (item.affected_items || 0) + '</p>';
                    historyHtml += '<p><strong>Status:</strong> ' + (item.status || 'unknown') + '</p>';
                    historyHtml += '</div>';
                    if (item.backup_file) {
                        historyHtml += '<div>';
                        historyHtml += '<button type="button" class="button button-secondary wtsr-rollback-btn" data-operation-id="' + item.id + '" data-backup="' + item.backup_file + '">Rollback</button>';
                        historyHtml += '</div>';
                    }
                    historyHtml += '</div>';
                    historyHtml += '</div>';
            ...
            $('.wtsr-modal-body').html(historyHtml);
        },
        ...
        handleRollback: function(e) {
            ...
            var $btn = $(e.target);
            var backupFile = $btn.data('backup');
            var operationId = $btn.data('operation-id');
            
            if (!backupFile && !operationId) {
                this.showMessage('No backup information found for rollback', 'error');
                return;
            }
            
            if (!confirm('Are you sure you want to rollback this replacement? This action cannot be undone.')) {
                return;
            }
            ...
            .done(function(response) {
                if (response && response.success) {
                    self.showMessage(response.data.message || 'Rollback completed successfully', 'success');
                    $('#wtsr-history-modal').hide();
                } else {
                    self.showMessage(response.data.message || 'Rollback failed', 'error');
                }
            })
            .fail(function() {
                self.showMessage('Rollback request failed', 'error');
            })
            .always(function() {
        ...
        showReplacementSummary: function(results, formData) {
            var summaryHtml = '<div class="wtsr-replacement-summary" style="margin: 20px 0; padding: 15px; border: 1px solid #ddd; background: #f9f9f9;">';
            summaryHtml += '<h3 style="margin-top: 0;">Replacement Completed</h3>';
            summaryHtml += '<p><strong>Search Text:</strong> "' + this.escapeHtml(formData.search_text) + '"</p>';
            summaryHtml += '<p><strong>Replace Text:</strong> "' + this.escapeHtml(formData.replace_text) + '"</p>';
            summaryHtml += '<p><strong>Total Replacements:</strong> ' + results.total_replacements + '</p>';
            
            if (results.database_items > 0) {
                summaryHtml += '<p><strong>Database Items:</strong> ' + results.database_items + '</p>';
            }
            if (results.file_items > 0) {
                summaryHtml += '<p><strong>Files:</strong> ' + results.file_items + '</p>';
            }
            
            if (results.errors && results.errors.length > 0) {
                summaryHtml += '<p><strong>Errors:</strong></p><ul>';
                results.errors.forEach(function(error) {
                    summaryHtml += '<li style="color: #d63384;">' + error + '</li>';
                });
                summaryHtml += '</ul>';
            }
            
            summaryHtml += '<div style="margin-top: 15px;">';
            summaryHtml += '<button type="button" id="wtsr-view-history" class="button button-secondary" style="margin-right: 10px;">View Replace History</button>';
            if (results.backup_file) {
                summaryHtml += '<button type="button" class="button button-secondary wtsr-rollback-btn" data-backup="' + this.escapeHtml(results.backup_file) + '">Rollback Changes</button>';
            }
            summaryHtml += '</div>';
            summaryHtml += '</div>';
            
            $('#wtsr-messages').append(summaryHtml);
            
            // Scroll to summary
            $('html, body').animate({
                scrollTop: $('.wtsr-replacement-summary').last().offset().top - 100
            }, 300);
        }
    };
