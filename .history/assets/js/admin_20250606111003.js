jQuery(document).ready(function($) {
    ...
    var WTSR = {
        ...
        getAllResults: function() {
            // Get all results from the current search results
            var allResults = [];
        ...
        performReplace: function() {
            ...
            $.post(wtsr_ajax.ajax_url, formData)
                .done(function(response) {
                    ...
                    if (response && response.success) {
                        // Show detailed replacement results instead of simple message
        ...
        closeModal: function(e) {
            ...
            $('#wtsr-modal').hide();
            $('#wtsr-results-modal').hide(); // Also close results modal if exists
        },
        ...
        showReplacementResults: function(results, searchText, replaceText) {
            var self = this;
            
            // Create results modal HTML
            var modalHtml = '<div id="wtsr-results-modal" class="wtsr-modal" style="display: block;">' +
                '<div class="wtsr-modal-content" style="max-width: 800px;">' +
                    ...
                    '<div class="wtsr-modal-body">' +
                        '<div class="wtsr-results-summary">' +
                            ...
                            '<p><strong>Total Items Replaced:</strong> ' + results.total_replacements + '</p>' +
                        '</div>' +
            ...
            this.populateReplacedItemsList(results);
            
            // Bind close events
            $('#wtsr-results-modal .wtsr-modal-close, #wtsr-close-results').on('click', function() {
                $('#wtsr-results-modal').hide();
            });
            
            // Close on background click
            $('#wtsr-results-modal').on('click', function(e) {
                if (e.target === this) {
                    $(this).hide();
                }
            });
        ...
        populateReplacedItemsList: function(results) {
            var itemsHtml = '';
            var replacedItems = this.getReplacedItemsDetails();
            ...
            if (replacedItems.length === 0) {
                ...
                replacedItems.forEach(function(item) {
                    ...
                    itemsHtml += '<td>' + self.escapeHtml(item.title) + '</td>';
                    itemsHtml += '<td>';
                    ...
                    if (item.view_url && item.type === 'post') {
