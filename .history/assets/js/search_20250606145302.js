jQuery(document).ready(function($) {
    ...
    var WTSR = {
        ...
        renderResults: function(results, startIndex) {
            ...
            console.log('WTSR: Rendering', results.length, 'results starting from index', startIndex);
            
            // Use document fragment for better performance
            results.forEach(function(item, index) {
                // Use continuous index based on current page and position
            ...
            console.log('WTSR: Rendered results HTML updated');
        ...
        performSearch: function() {
            ...
            $.post(wtsr_ajax.ajax_url, formData)
                ...
                .always(function() {
                    ...
                    // Final cleanup: ensure selection state is reset
                    self.selectedItems = [];
        ...
        updateReplaceButtonState: function() {
            ...
            if (hasSelection) {
                ...
                // Ensure no items are visually selected when count is 0
                $('.wtsr-item-checkbox').prop('checked', false);
            }
        },
            ...
            $.post(wtsr_ajax.ajax_url, formData)
                .done(function(response) {
                    if (response && response.success) {
                        ...
                        // Reset selection immediately after successful replace
                        self.selectedItems = [];
                        self.updateReplaceButtonState();
                        
                        // Show success message
                        var successMessage = 'Successfully replaced ' + replacedItemsDetails.length + ' item(s). Results list has been refreshed.';
                        ...
                        // Remove replaced items from results and refresh the list
        ...
        removeReplacedItems: function() {
            ...
            console.log('WTSR: removeReplacedItems called with selectedItems:', this.selectedItems);
            ...
            console.log('WTSR: Sorted indices to remove:', sortedIndices);
            
            // Clear selection state immediately
            this.selectedItems = [];
            this.updateReplaceButtonState();
            
            // Remove items from the actual search results data
            this.removeItemsFromSearchResults(sortedIndices);
            
            // Recalculate total results
            var databaseCount = (this.searchResults.database && Array.isArray(this.searchResults.database)) ? this.searchResults.database.length : 0;
            var filesCount = (this.searchResults.files && Array.isArray(this.searchResults.files)) ? this.searchResults.files.length : 0;
            this.totalResults = databaseCount + filesCount;
            
            console.log('WTSR: New total results after removal:', this.totalResults);
            
            // Update summary text
            $('#wtsr-summary-text').text('Found ' + this.totalResults + ' matches (after replacements)');
            
            // Check if current page is still valid
            var totalPages = this.getTotalPages();
            if (this.currentPage > totalPages && totalPages > 0) {
                this.currentPage = totalPages;
                console.log('WTSR: Adjusted current page to:', this.currentPage);
            ...
            if (this.totalResults === 0) {
                ...
                $('.wtsr-pagination-controls').hide();
                console.log('WTSR: No more results, showing completion message');
            } else {
                console.log('WTSR: Calling displayCurrentPage to refresh results');
                this.displayCurrentPage();
            }
            
            // Final state update to ensure everything is clean
            this.selectedItems = [];
            this.updateReplaceButtonState();
            
            console.log('WTSR: removeReplacedItems completed');
        },
        ...
        removeItemsFromSearchResults: function(indices) {
            var self = this;
            
            console.log('WTSR: Removing items with indices:', indices);
            console.log('WTSR: Before removal - Database items:', this.searchResults.database ? this.searchResults.database.length : 0);
            console.log('WTSR: Before removal - File items:', this.searchResults.files ? this.searchResults.files.length : 0);
            
            // Get all results as a flat array to map indices correctly
            var allResults = [];
            var databaseCount = 0;
            var filesCount = 0;
            ...
            if (this.searchResults.database && Array.isArray(this.searchResults.database)) {
                allResults = allResults.concat(this.searchResults.database);
                databaseCount = this.searchResults.database.length;
            }
            if (this.searchResults.files && Array.isArray(this.searchResults.files)) {
                allResults = allResults.concat(this.searchResults.files);
                filesCount = this.searchResults.files.length;
            }
            
            console.log('WTSR: Total items before removal:', allResults.length);
            
            // Create arrays to track which items to remove from each category
            var databaseIndicesToRemove = [];
            var filesIndicesToRemove = [];
            
            // Map global indices to category-specific indices
            indices.forEach(function(globalIndex) {
                if (globalIndex < databaseCount) {
                    // This is a database item
                    databaseIndicesToRemove.push(globalIndex);
                    console.log('WTSR: Will remove database item at index:', globalIndex);
                } else {
                    // This is a file item
                    var fileIndex = globalIndex - databaseCount;
                    filesIndicesToRemove.push(fileIndex);
                    console.log('WTSR: Will remove file item at index:', fileIndex, '(global index:', globalIndex, ')');
                }
            });
            
            // Sort in descending order to remove from end to beginning
            databaseIndicesToRemove.sort(function(a, b) { return b - a; });
            filesIndicesToRemove.sort(function(a, b) { return b - a; });
            
            // Remove items from database results
            if (this.searchResults.database && Array.isArray(this.searchResults.database)) {
                databaseIndicesToRemove.forEach(function(index) {
                    if (index >= 0 && index < self.searchResults.database.length) {
                        console.log('WTSR: Removing database item at index:', index);
                        self.searchResults.database.splice(index, 1);
                    }
                });
            }
            
            // Remove items from files results
            if (this.searchResults.files && Array.isArray(this.searchResults.files)) {
                filesIndicesToRemove.forEach(function(index) {
                    if (index >= 0 && index < self.searchResults.files.length) {
                        console.log('WTSR: Removing file item at index:', index);
                        self.searchResults.files.splice(index, 1);
                    }
                });
            }
            
            console.log('WTSR: After removal - Database items:', this.searchResults.database ? this.searchResults.database.length : 0);
            console.log('WTSR: After removal - File items:', this.searchResults.files ? this.searchResults.files.length : 0);
        }
    };
