...
<div class="wrap wtsr-admin-page">
    ...
    <div class="wtsr-container">
        ...
        <div class="wtsr-search-form">
            ...
            <!-- Debug Information -->
            <div id="wtsr-debug-info" style="background: #f0f0f1; border: 1px solid #c3c4c7; padding: 10px; margin-bottom: 20px; display: none;">
                <h4>调试信息</h4>
                <p><strong>AJAX URL:</strong> <span id="debug-ajax-url"></span></p>
                <p><strong>Nonce:</strong> <span id="debug-nonce"></span></p>
                <p><strong>用户权限:</strong> <?php echo current_user_can('manage_options') ? '管理员' : '非管理员'; ?></p>
                <p><strong>WordPress版本:</strong> <?php echo get_bloginfo('version'); ?></p>
                <p><strong>PHP版本:</strong> <?php echo PHP_VERSION; ?></p>
                <button type="button" id="wtsr-test-ajax" class="button button-secondary">测试AJAX连接</button>
            </div>
            
            <p>
                <button type="button" id="wtsr-toggle-debug" class="button button-secondary" style="float: right;">
                    显示调试信息
                </button>
            ...
            <form id="wtsr-search-form">
                <table class="form-table">
                        ...
                        <td>
                            <fieldset>
                                ...
                                <label for="search_acf" style="color: #999;">
                                    ...
                                    <?php _e('ACF字段 (需要安装ACF插件)', 'worldteam-search-replace'); ?>
                                </label><br>
                                <?php endif; ?>
                    ...
                    <tr>
                        <th scope="row"><?php _e('性能设置', 'worldteam-search-replace'); ?></th>
                        <td>
                            <fieldset>
                                <legend class="screen-reader-text"><?php _e('性能优化选项', 'worldteam-search-replace'); ?></legend>
                                
                                <p>
                                    <label for="wtsr-max-files"><?php _e('最大文件搜索数量:', 'worldteam-search-replace'); ?></label>
                                    <input type="number" id="wtsr-max-files" name="max_files" value="500" min="100" max="2000" style="width: 80px;">
                                    <span class="description"><?php _e('限制文件搜索数量以提高性能（建议: 500-1000）', 'worldteam-search-replace'); ?></span>
                                </p>
                                
                                <p>
                                    <label for="wtsr-db-limit"><?php _e('数据库查询限制:', 'worldteam-search-replace'); ?></label>
                                    ...
                                    <span class="description"><?php _e('每页显示的数据库结果数量', 'worldteam-search-replace'); ?></span>
                                </p>
                                
                                <p>
                                    <label>
                                        <input type="checkbox" id="wtsr-skip-large-files" name="skip_large_files" value="1" checked>
                                        <?php _e('跳过大文件 (>2MB)', 'worldteam-search-replace'); ?>
                                    </label>
                                    <span class="description"><?php _e('跳过大文件以提高搜索速度', 'worldteam-search-replace'); ?></span>
                                </p>
                                
                                <p>
                                    <label>
                                        <input type="checkbox" id="wtsr-enable-cache" name="enable_cache" value="1">
                                        <?php _e('启用搜索缓存', 'worldteam-search-replace'); ?>
                                    </label>
                                    <span class="description"><?php _e('缓存搜索结果以提高重复搜索速度（实验性功能）', 'worldteam-search-replace'); ?></span>
                                </p>
                            </fieldset>
                        </td>
                    </tr>
