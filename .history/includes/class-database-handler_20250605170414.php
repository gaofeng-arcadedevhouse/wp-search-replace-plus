...
class WTSR_Database_Handler {
    ...
    private function replaceInACF($item, $search_text, $replace_text, $options) {
        ...
        $current_value = get_field($field_key, $item['id']);
        
        if (is_string($current_value)) {
            $new_value = str_ireplace($search_text, $replace_text, $current_value);
            update_field($field_key, $new_value, $item['id']);
            error_log('WTSR: ACF field updated - Post ID: ' . $item['id'] . ', Field: ' . $field_key);
        } else {
            error_log('WTSR: ACF field is not a string, skipping replacement');
        }
    }
} 