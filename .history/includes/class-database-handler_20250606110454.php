...
class WTSR_Database_Handler {
    ...
    public static function createTables() {
        ...
        // Table for tracking active replace operations
        ...
        // Table for tracking individual item replacements
        $operations_items_table = $wpdb->prefix . 'wtsr_operation_items';
        
        $sql2 = "CREATE TABLE $operations_items_table (
            id bigint(20) NOT NULL AUTO_INCREMENT,
        ...
        update_option('wtsr_db_version', '1.1');
    }
    ...
    // Enhanced replace method with history tracking
    public function replace($search_text, $replace_text, $items, $options) {
        ...
        $operation_id = $this->startOperation($search_text, $replace_text, $items, $options);
        
        try {
            foreach ($items as $item) {
                try {
                    if ($item['type'] === 'post') {
                        $result = $this->replaceInPost($item, $search_text, $replace_text, $options, $operation_id);
                        if ($result) {
                            $replaced_items++;
                            $database_items++;
                        }
                    } elseif ($item['type'] === 'acf') {
                        $result = $this->replaceInACF($item, $search_text, $replace_text, $options, $operation_id);
                        if ($result) {
                            $replaced_items++;
                            $database_items++;
                        }
                    } elseif ($item['type'] === 'file') {
                        // File replacements will be handled by File Scanner
                        $file_items++;
                    }
                } catch (Exception $e) {
                    $error_msg = sprintf(
                    ...
                    // Log error to operation tracking
                    $this->logOperationError($operation_id, $item, $error_msg);
                }
            }
            
            // Complete operation tracking
            $this->completeOperation($operation_id, $replaced_items, $errors);
            
            return array(
                'replaced_items' => $replaced_items,
                'database_items' => $database_items,
                'file_items' => $file_items,
                'total_replacements' => $replaced_items,
            ...
            $this->failOperation($operation_id, $e->getMessage());
            throw $e;
        }
    }
    
    private function startOperation($search_text, $replace_text, $items, $options) {
        global $wpdb;
        
        $operations_table = $wpdb->prefix . 'wtsr_operations';
        ...
        if ($result === false) {
            throw new Exception('Failed to start operation tracking');
        }
        
        return $wpdb->insert_id;
    }
    
    private function completeOperation($operation_id, $affected_items, $errors) {
        global $wpdb;
    ...
    private function failOperation($operation_id, $error_message) {
        global $wpdb;
        ...
        $wpdb->update(
            ...
            array(
                'status' => 'failed',
                'errors' => json_encode(array($error_message)),
                'completed_at' => current_time('mysql')
            ),
            array('id' => $operation_id),
            array('%s', '%s', '%s'),
    ...
    private function logOperationError($operation_id, $item, $error_message) {
        global $wpdb;
        
        $items_table = $wpdb->prefix . 'wtsr_operation_items';
    ...
    private function replaceInPost($item, $search_text, $replace_text, $options, $operation_id) {
        ...
        if ($updated) {
            ...
            $this->logOperationItem($operation_id, $item, $post->post_content, $post_data['post_content']);
            return true;
        }
        return false;
    }
    
    private function replaceInACF($item, $search_text, $replace_text, $options, $operation_id) {
        ...
        if ($current_value !== null && $current_value !== false) {
            ...
            if ($updated) {
                ...
                $this->logOperationItem($operation_id, $item, $current_value, $new_value);
                return true;
            } else {
    ...
    private function logOperationItem($operation_id, $item, $original_value, $new_value) {
        global $wpdb;
    ...
    public function getOperationHistory($limit = 20) {
        ...
        foreach ($results as &$operation) {
            $operation['created_at'] = date('Y-m-d H:i:s', strtotime($operation['created_at']));
            if ($operation['completed_at']) {
                $operation['completed_at'] = date('Y-m-d H:i:s', strtotime($operation['completed_at']));
            }
            $operation['options'] = json_decode($operation['options'], true);
            $operation['errors'] = json_decode($operation['errors'], true);
        }
    ...
    public function getOperationDetails($operation_id) {
        ...
        $operation = $wpdb->get_row($wpdb->prepare(
            "SELECT * FROM $operations_table WHERE id = %d AND user_id = %d",
            $operation_id,
            get_current_user_id()
        ), ARRAY_A);
        ...
        $items = $wpdb->get_results($wpdb->prepare(
            "SELECT * FROM $items_table WHERE operation_id = %d ORDER BY processed_at DESC",
            $operation_id
        ), ARRAY_A);
    ...
    public function rollbackOperation($operation_id) {
        global $wpdb;
        
        $operation = $this->getOperationDetails($operation_id);
        if (!$operation) {
            throw new Exception('Operation not found or access denied');
        }
        ...
        $rollback_operation_id = $this->startOperation(
            'ROLLBACK: ' . $operation['search_text'], 
            $operation['replace_text'], 
            $operation['items'], 
            array_merge($operation['options'], array('rollback' => true))
        );
        ...
        try {
            foreach ($operation['items'] as $item) {
                ...
                try {
                    if ($item['item_type'] === 'post') {
                        $this->rollbackPostItem($item);
                        $rolled_back_items++;
                    } elseif ($item['item_type'] === 'acf') {
                        $this->rollbackACFItem($item);
                        $rolled_back_items++;
                    }
                } catch (Exception $e) {
                    $error_msg = sprintf(
                        'Failed to rollback %s item %d: %s', 
                        $item['item_type'], 
                        $item['item_id'], 
                        $e->getMessage()
                    );
            ...
            $this->completeOperation($rollback_operation_id, $rolled_back_items, $errors);
            ...
            $this->failOperation($rollback_operation_id, $e->getMessage());
            throw $e;
        }
    }
    
    private function rollbackPostItem($item) {
        $post = get_post($item['item_id']);
        if (!$post) {
            throw new Exception('Post not found');
        }
        
        $post_data = array('ID' => $item['item_id']);
        $updated = false;
        
        // Restore original content if it was changed
        if ($item['field_name'] === 'post_content' && $item['original_value'] !== null) {
            $post_data['post_content'] = $item['original_value'];
            $updated = true;
        } elseif ($item['field_name'] === 'post_title' && $item['original_value'] !== null) {
            $post_data['post_title'] = $item['original_value'];
            $updated = true;
        } else {
            // For general post updates, restore content
            $post_data['post_content'] = $item['original_value'];
            $updated = true;
        }
    ...
    private function rollbackACFItem($item) {
        if (!function_exists('update_field')) {
            throw new Exception('ACF not available for rollback');
        }
        
        $field_key = $item['field_key'] ?: $item['field_name'];
        $post_id = $item['item_id'];
        $original_value = $item['original_value'];
        
        // Decode if the original value was JSON
        if (is_string($original_value) && (strpos($original_value, '{') === 0 || strpos($original_value, '[') === 0)) {
        ...
        $result = update_field($field_key, $original_value, $post_id);
        if (!$result) {
            throw new Exception('Failed to restore ACF field value');
        }
