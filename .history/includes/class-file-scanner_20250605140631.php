...
class WTSR_File_Scanner {
    ...
    private $excluded_directories = array('.git', '.svn', 'node_modules', 'vendor', '.idea');
    
    public function search($search_text, $search_scope, $options) {
        $results = array();
        
        // Search in theme files
        if (in_array('theme', $search_scope)) {
            ...
            // Also search child theme if exists
            if (is_child_theme()) {
        ...
        if (in_array('plugins', $search_scope)) {
            $plugins_results = $this->searchInDirectory(
                WP_PLUGIN_DIR,
                $search_text,
                $options,
                'plugin'
            );
            $results = array_merge($results, $plugins_results);
        }
        
        return $results;
    }
    
    private function searchInDirectory($directory, $search_text, $options, $type) {
        $results = array();
    ...
    private function isExcludedPath($path) {
        foreach ($this->excluded_directories as $excluded) {
            if (strpos($path, $excluded . '/') !== false || strpos($path, $excluded . '\\') !== false) {
                return true;
            }
        }
        return false;
    ...
    public function replace($search_text, $replace_text, $items, $options) {
        ...
        foreach ($items as $item) {
            ...
            try {
                ...
                $errors[] = sprintf(
                    __('替换文件 %s 失败: %s', 'worldteam-search-replace'),
                    $item['relative_path'],
                    $e->getMessage()
                );
            }
    ...
    private function replaceInFile($item, $search_text, $replace_text, $options) {
        ...
        if (!is_writable($file_path)) {
            throw new Exception(__('文件不可写', 'worldteam-search-replace'));
        }
        
        // Read current content
        $content = file_get_contents($file_path);
        if ($content === false) {
            throw new Exception(__('无法读取文件内容', 'worldteam-search-replace'));
        }
        ...
        if (!copy($file_path, $backup_path)) {
            throw new Exception(__('无法创建文件备份', 'worldteam-search-replace'));
        }
        
        // Write new content
        if (file_put_contents($file_path, $new_content) === false) {
            throw new Exception(__('无法写入文件', 'worldteam-search-replace'));
        }
    ...
    public function restoreFileBackup($backup_info) {
        ...
        if (!is_writable($backup_info['original_file'])) {
            throw new Exception(__('原文件不可写', 'worldteam-search-replace'));
        }
        
        if (!copy($backup_info['backup_file'], $backup_info['original_file'])) {
            throw new Exception(__('恢复失败', 'worldteam-search-replace'));
        }
