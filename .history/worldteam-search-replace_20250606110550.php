...
class WorldTeamSearchReplace {
    ...
    public function init() {
        ...
        add_action('wp_ajax_wtsr_get_history', array($this, 'ajaxGetHistory'));
        add_action('wp_ajax_wtsr_rollback', array($this, 'ajaxRollback'));
    ...
    public function ajaxGetHistory() {
        error_log('WTSR: AJAX get history handler called');
        
        try {
            // Check nonce
            if (!wp_verify_nonce($_POST['nonce'] ?? '', 'wtsr_nonce')) {
                error_log('WTSR: Nonce verification failed for get history');
                wp_send_json_error(array(
                    'message' => __('Security verification failed, please refresh the page and try again', 'worldteam-search-replace')
                ));
                return;
            ...
            // Check permissions
            if (!current_user_can('manage_options')) {
                error_log('WTSR: Permission check failed for get history');
                wp_send_json_error(array(
                    'message' => __('Insufficient permissions', 'worldteam-search-replace')
                ));
                return;
            ...
            error_log('WTSR: Getting operation history');
            $database_handler = new WTSR_Database_Handler();
            $history = $database_handler->getOperationHistory();
            
            wp_send_json_success($history);
            
        } catch (Exception $e) {
            error_log('WTSR: Exception in get history AJAX handler: ' . $e->getMessage());
            wp_send_json_error(array(
                'message' => sprintf(__('Error occurred while fetching history: %s', 'worldteam-search-replace'), $e->getMessage())
            ));
        } catch (Error $e) {
            error_log('WTSR: Fatal error in get history AJAX handler: ' . $e->getMessage());
            wp_send_json_error(array(
                'message' => sprintf(__('System error occurred while fetching history: %s', 'worldteam-search-replace'), $e->getMessage())
            ));
        }
    }
    
    public function ajaxRollback() {
        error_log('WTSR: AJAX rollback handler called');
        
        try {
            // Check nonce
            if (!wp_verify_nonce($_POST['nonce'] ?? '', 'wtsr_nonce')) {
                error_log('WTSR: Nonce verification failed for rollback');
                wp_send_json_error(array(
                    'message' => __('Security verification failed, please refresh the page and try again', 'worldteam-search-replace')
                ));
                return;
            ...
            // Check permissions
            if (!current_user_can('manage_options')) {
                error_log('WTSR: Permission check failed for rollback');
                wp_send_json_error(array(
                    'message' => __('Insufficient permissions', 'worldteam-search-replace')
                ));
                return;
            ...
            if (!$operation_id && !$backup_file) {
                wp_send_json_error(array(
                    'message' => __('Missing operation ID or backup file information', 'worldteam-search-replace')
                ));
                return;
            }
            
            error_log('WTSR: Starting rollback process');
            $database_handler = new WTSR_Database_Handler();
            
            if ($operation_id) {
                $result = $database_handler->rollbackOperation($operation_id);
            } else {
                // Handle file rollback (to be implemented)
                wp_send_json_error(array(
                    'message' => __('File rollback not yet implemented', 'worldteam-search-replace')
                ));
                return;
            }
            
            wp_send_json_success(array(
                'message' => sprintf(__('Successfully rolled back %d items', 'worldteam-search-replace'), $result['rolled_back_items']),
                'result' => $result
            ));
            
        } catch (Exception $e) {
            error_log('WTSR: Exception in rollback AJAX handler: ' . $e->getMessage());
            wp_send_json_error(array(
                'message' => sprintf(__('Error occurred during rollback: %s', 'worldteam-search-replace'), $e->getMessage())
            ));
        } catch (Error $e) {
            error_log('WTSR: Fatal error in rollback AJAX handler: ' . $e->getMessage());
            wp_send_json_error(array(
                'message' => sprintf(__('System error occurred during rollback: %s', 'worldteam-search-replace'), $e->getMessage())
            ));
        }
    }
